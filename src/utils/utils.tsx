...
export const extractFormikError = (errors: unknown, touched: unknown, names: string[]): string =>
  names
    .map((name) => {
...
export const unit8ToHexPrefixed = (key: Uint8Array) => "0x" + Buffer.from(key).toString("hex");
...
export const getKeys = (password: string, accountIndex: number, vault: string | null) => {
  if (!vault)
    return {
  ...
  const decryptedVault = CryptoJS.AES.decrypt(vault.toString(), password).toString(CryptoJS.enc.Latin1);
...
export const findClosestBigger = (bills: IBill[], target: string): IBill | undefined => {
  return bills
...
export const getClosestSmaller = (bills: IBill[], target: string) => {
  ...
  return bills?.reduce((acc: IBill, obj: IBill) => {
    ...
    const currentAbsDiff = accValue > targetInt ? accValue - targetInt : targetInt - accValue;
    return absDiff < currentAbsDiff ? obj : acc;
  });
};

export const getOptimalBills = (amount: string, billsArr: IBill[], feeAmount: bigint = 0n): IBill[] => {
  if (!billsArr) {
    return [];
  }
...
export function handleBillSelection(
  ...
  const billsSumDifference =
    ...
    (optimalBills.length < 1 || !feeAmount ? 0n : BigInt(optimalBills.length) * feeAmount);

  const billToSplit = billsSumDifference !== 0n ? findClosestBigger(optimalBills, billsSumDifference.toString()) : null;

  const billsToTransfer = billToSplit ? optimalBills?.filter((bill) => bill.id !== billToSplit?.id) : optimalBills;

  const splitBillAmount = billToSplit ? BigInt(billToSplit.value) - billsSumDifference : null;
...
export const convertToWholeNumberBigInt = (val: string | number, decimals: number): bigint => {
  ...
  const fullNumber =
    decimalDifference > 0 ? numStrWithoutDecimal + "0".repeat(decimalDifference) : numStrWithoutDecimal;
...
export const separateDigits = (numStr: string) => {
  ...
  if (Number(decimalPart?.length) > 0) {
    const formattedDecimalPart = decimalPart.replace(/0+$/, "").replace(/(\d{3})(?=\d)/g, "$1'");
...
export const invalidateAllLists = async (pubKey: string, assetTypeId: string, queryClient: QueryClient) => {
  ...
  return (
    NFTsList?.map((nft) => {
      return Object.assign(nft, {
        ...
        iconImage: nft.icon,
        amountOfSameType: NFTsList?.filter((obj: IListTokensResponse) => obj.typeId === nft.typeId)?.length || "0",
      });
    }) || []
  );
};
...
const getUpdatesUTPFungibleTokens = (fungibleTokensList: IListTokensResponse[] | undefined = []) => {
  ...
  if (fungibleTokensList.length >= 1) {
    for (const token of fungibleTokensList) {
      if (!typeIDs.includes(token.typeId)) {
        ...
        for (const resultToken of userTokens) {
          if (resultToken.typeId === token.typeId && token?.value) {
            resultToken.amount = (BigInt(resultToken.amount) + BigInt(token.value!)).toString();
          }
        }
      }
    }
  ...
  return (
    userTokens?.map((obj: IListTokensResponse) => ({
      ...
      isSendable: isTokenSendable(),
      iconImage: obj.icon,
      UIAmount: separateDigits(addDecimal(obj.amount || "0", obj.decimals || 0)),
    })) || []
  );
};
...
export const getUpdatedFungibleAssets = (
  ...
  const ALPHABalance = String(
    balances?.find((balance: any) => balance?.data?.pubKey === activeAccountId)?.data?.balance || "0",
  );
...
const sortByTypeId = (arr: any) =>
  arr?.sort((a: any, b: any) => (a.typeId < b.typeId ? -1 : a.typeId > b.typeId ? 1 : 0));

const filterUniqueTypes = (arr: any) =>
  arr?.filter(({ typeId }: any, index: any, array: any) => index === 0 || typeId !== array[index - 1].typeId);

const sortBySymbol = (arr: any) =>
  arr?.sort((a: any, b: any) => (a.symbol < b.symbol ? -1 : a.symbol > b.symbol ? 1 : 0));
...
export const getFungibleAssetsAmount = (account: IAccount, decimals: number, assetTypeId: string) =>
  addDecimal(
    BigInt(account?.assets?.fungible?.find((asset) => asset.typeId === assetTypeId)?.amount || "0").toString() || "0",
    Number(decimals),
  );
...
export const Base64imageComponent: React.FC<{
  ...
  const imageUrl = isImage(base64Data.data, base64Data.type) ? base64Data.data : null;
...
export const clearStorage = () => localStorageKeys.map((key) => localStorage.removeItem(key));

export const createEllipsisString = (id: string, firstCount: number, lastCount: number): string => {
...
export const isBillLocked = (consolidationTargetUnit: IBill, asset: IBill, DCBills: IBill[]) => {
  return (
    (consolidationTargetUnit?.id === asset.id &&
      DCBills?.length >= 1 &&
      Boolean(DCBills?.find((cb: IBill) => cb.targetUnitId === consolidationTargetUnit?.id))) ||
    (DCBills?.length >= 1 &&
