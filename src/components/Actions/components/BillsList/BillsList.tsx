...
function BillsList(): JSX.Element | null {
  ...
  const addInterval = () => {
    ...
    swapInterval.current = setInterval(() => {
      ...
      getBlockHeight(activeNetwork.backendAPI).then((blockData) => {
      ...
      getBlockHeight(activeNetwork.backendAPI).then((blockData) =>
        sortedListByID.map(async (bill: IBill, idx) => {
          ...
          if (secp.verify(signature[0], msgHash, hashingPublicKey)) {
            ...
            makeTransaction(dataWithProof, activeNetwork.moneyPartitionAPI)
    ...
    <>
      <div className="dashboard__info-col active relative bills-list">
        <Button
          onClick={() => {
            queryClient.invalidateQueries(["billsList", activeAccountId, activeNetwork.backendAPI]);
            queryClient.invalidateQueries(["balance", activeAccountId, activeNetwork.backendAPI]);
          }}
