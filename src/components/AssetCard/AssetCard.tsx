...
import { ICRight, ICFeeLimit } from "../../css/icons";
...
export const AssetCard = ({ isMini, asset, isFungible }: IAssetCardProps) => {
  const className = classNames("asset-card", {
  ...
  const amount = isFungible ? (asset as IFungibleAsset).UIAmount : (asset as INFTAsset).amountOfSameType;

  const label = isFungible ? (asset as IFungibleAsset).id : (asset as INFTAsset).symbol;

  return (
    <div className={className}>
      <div className="asset-card-header">
        <AssetCardIcon asset={asset} isTypeListItem={false} />
      </div>
      ...
      <div className="asset-card-footer">
        <Button variant="icon">
          <ICFeeLimit />
          {!isMini && <div className="pad-8-l">Transaction fee limit</div>}
        </Button>
