import { TransferFeeCreditTransactionOrder } from "@alphabill/alphabill-js-sdk/lib/fees/transactions/TransferFeeCredit";
...
import { IBill, IFeeCreditForm } from "../../types/Types";
...
export default function TransferFeeCredit(): JSX.Element | null {
  const { setIsActionsViewVisible, isActionsViewVisible, account, unlockedBillsList, setPreviousView } = useApp();
  ...
  const getAvailableAmount = useCallback(
    (decimals: number) => addDecimal(getBillsSum(billsArr).toString() || "0", Number(decimals)),
    [billsArr],
  );

  const [availableAmount, setAvailableAmount] = useState<string>(getAvailableAmount(AlphaDecimals || 0));
  ...
  const removePollingInterval = useCallback(() => {
    ...
    setIsSending(false);
    isFeeCreditAdded.current && setIsActionsViewVisible(false);

    isFeeCreditAdded.current = false;
  }, [setIsActionsViewVisible]);

  const addPollingInterval = useCallback(
    (txProof: TransactionRecordWithProof<TransferFeeCreditTransactionOrder>) => {
  ...
  const handleSubmit = useCallback(
    async (values: IFeeCreditForm, setErrors: (errors: FormikErrors<IFeeCreditForm>) => void) => {
      ...
      try {
        convertedAmount = convertToWholeNumberBigInt(values.amount, AlphaDecimals);
      } catch (error) {
        setIsSending(false);
        return setErrors({
      ...
      <Formik
        ...
        validationSchema={Yup.object().shape({
          ...
          amount: Yup.string()
            ...
            .test("test exceeds assets", "Amount with fees exceeds available assets", (value: string | undefined) => {
              setTransferredBillsCount(0);
              let convertedAmount: bigint;
              if (!value) return false;

              try {
                convertedAmount = convertToWholeNumberBigInt(value, AlphaDecimals);
              } catch (error) {
                return false;
              }
            ...
            .test(
              ...
              (value: string | undefined) => {
                ...
                const minAmount = MaxTransactionFee * 3n * BigInt(optimalBills.length);

                setMinFeeTransferAmount(addDecimal(minAmount.toString(), Number(AlphaDecimals)));
                  ...
                  <Textfield
                    ...
                    desc={
                      ...
                      (transferredBillsCount >= 1
                        ? " - fee for transfer " +
                          addDecimal((transferredBillsCount * 2).toString(), AlphaDecimals).toString() +
                          " ALPHA"
                        : "")
                    }
                    type="text"
